...
class LSTM_network:
    ...
    def __init__(self, n_hidden, embedding_dim, n_classes, weights=None, debug=False):
        ...
        if weights is not None:
            ...
            self.W_dense_l1_fw = tf.constant(weights[6][:self.n_hidden], dtype=tf.float64)
            self.W_dense_l1_bw = tf.constant(weights[6][self.n_hidden:], dtype=tf.float64)
                       
            self.W_dense_l2_fw = tf.constant(weights[7][:self.n_hidden], dtype=tf.float64)
            self.W_dense_l2_bw = tf.constant(weights[7][self.n_hidden:], dtype=tf.float64)

            self.W_dense_l3_fw = tf.constant(weights[8][:self.n_hidden], dtype=tf.float64)
            self.W_dense_l3_bw = tf.constant(weights[8][self.n_hidden:], dtype=tf.float64)
            
            self.W_dense_l4_fw = tf.constant(weights[9][:self.n_hidden], dtype=tf.float64)
            self.W_dense_l4_bw = tf.constant(weights[9][self.n_hidden:], dtype=tf.float64)
            
            self.W_dense_l5_fw = tf.constant(weights[10][:self.n_hidden], dtype=tf.float64)
            self.W_dense_l5_bw = tf.constant(weights[10][self.n_hidden:], dtype=tf.float64)

            self.W_dense_l6_fw = tf.constant(weights[11][:self.n_hidden], dtype=tf.float64)
            self.W_dense_l6_bw = tf.constant(weights[11][self.n_hidden:], dtype=tf.float64)
            
            self.W_dense_l7_fw = tf.constant(weights[12][:self.n_hidden], dtype=tf.float64)
            self.W_dense_l7_bw = tf.constant(weights[12][self.n_hidden:], dtype=tf.float64)

            self.b_dense = tf.constant(weights[13], dtype=tf.float64)
    ...
    def check_weights(self, weights):
        print("Number of weight class: ")
        print(len(weights))
        
        for z in range(len(weights)):
            print("SHAPE: ")
            print(weights[z].shape)
        ...
        ### This implementation has been done for custome LSTM network, this is static 

        assert weights[0].shape == weights[3].shape == (self.embedding_dim, 4 * self.n_hidden)
        assert weights[1].shape == weights[4].shape == (self.n_hidden, 4 * self.n_hidden)
        assert weights[2].shape == weights[5].shape == (4 * self.n_hidden, )
        
        assert weights[6].shape == (2 * self.n_hidden, self.n_hidden)
        assert weights[7].shape == weights[9].shape == weights[11].shape == (self.n_hidden,)
        assert weights[8].shape == weights[10].shape == (self.n_hidden, self.n_hidden)
        assert weights[12].shape == (self.n_hidden, self.n_classes)
        assert weights[13].shape == (self.n_classes,)
