...
# colums that are categorical and not binary yet: protocol_type (column 2), (column 3), flag (column 4).
...
# Modification of network implementation
...
# return_sequences=True, stateful=False,return_sequences=True, stateful=False,

model = Sequential()
model.add(Bidirectional(LSTM(122, input_shape=(1, 122))))
#model.add(LSTM(256, return_sequences=False, return_state = False))
model.add(Dropout(.1))

model.add(Dense(122, activation='selu'))
model.add(Dropout(.1))
...
model = Sequential()
model.add(LSTM(1, input_shape=(1, 122), return_sequences=False))
model.add(Dense(5, activation='softmax'))
print()
print()
